...
def analyze_hrv(ecg_signal, sampling_rate=1000, algorithm='vollmer'):
    ...
    """
    if algorithm == 'rri':
        from hrv_app.core.rri_rpeak import analyze_rri
...
def calculate_skna_metrics(raw_signal, fs, window_sec=5.0):
    # 帶通 500-1000 Hz、4 階 Butterworth
    sos_filter = signal.butter(4, [500.0, 1000.0], btype='band', fs=fs, output='sos')
    filtered_signal = signal.sosfilt(sos_filter, raw_signal)

    # 全波整流
    rectified_signal = np.abs(filtered_signal)

    # iSKNA：漏電積分器（τ=0.1s），輸出單位 μV·s（目前僅 aSKNA 有被使用）
    tau = 0.1
    ...
    # aSKNA：每 window_sec 一段的平均絕對電壓（μV）
    ...
    # overall_askna：全段平均 aSKNA（回報用的全局值）
